...
temple_src = Image.open(os.path.join(artifacts_dir, "media__1784197542737.jpg")).convert("RGBA")
bull_src = Image.open(os.path.join(artifacts_dir, "media__1784197542037.jpg")).convert("RGBA")
...
# 5. Add Temple in the middle-right (w: 920, h: 1200, x: 1840, y: 0)
temple_w, temple_h = 920, 1200
temple_crop = temple_src.resize((temple_w, temple_h), Image.Resampling.LANCZOS)
temple_mask = create_feathered_mask(temple_w, temple_h, fade_left=200, fade_right=200, fade_bottom=160)
desktop.paste(temple_crop, (1840, 0), temple_mask)
...
# Apply color grading to match aesthetic but make it brighter and colorful
# Saturation: 1.05 (extremely colorful)
converter = ImageEnhance.Color(desktop)
desktop = converter.enhance(1.05)

# Brightness: 1.08 (bright and crisp)
brightness_enhancer = ImageEnhance.Brightness(desktop)
desktop = brightness_enhancer.enhance(1.08)

# Apply softer dark blue gradient overlay vignette to keep it brighter
overlay = Image.new("RGBA", (canvas_w, canvas_h), (9, 12, 21, 0))
draw = ImageDraw.Draw(overlay)

for x in range(canvas_w):
    dist_from_center = abs(x - canvas_w // 2) / (canvas_w // 2)
    # Extremely light gradient to keep it bright and colorful
    alpha = int(10 + 25 * dist_from_center) 
...
# 6. Add Temple (swapped) (w: 1600, h: 640, x: 0, y: 1800)
m_temple_h = 640
m_temple_crop = temple_src.resize((m_w, m_temple_h), Image.Resampling.LANCZOS)
m_temple_mask = create_feathered_mask(m_w, m_temple_h, fade_top=120, fade_bottom=120)
mobile.paste(m_temple_crop, (0, 1800), m_temple_mask)
...
mobile = converter_m.enhance(1.05)

brightness_m = ImageEnhance.Brightness(mobile)
mobile = brightness_m.enhance(1.08)

# Mobile overlay (vertical gradient vignette) - softer
